...
$AdminId = $_SESSION["UserId"];/*!< a variable to save the admin id from the id of the user who started the session */
global $ConnectingDB;/*!< a variable that refers to the database we are connected to*/
$sql  = "SELECT * FROM admins WHERE id='$AdminId'";/*!< a variable to define an sql statement to get all the admins from the admins table in the database whose id='$AdminId' */
$stmt =$ConnectingDB->query($sql);/*!< a variable to perform the sql statement by the database */
while ($DataRows = $stmt->fetch()) {/*! looping over all the rows in the admins table in the database */
