...
<?php Confirm_Login(); ?>
<?php
$SarchQueryParameter = $_GET['id'];/*!< a variable to get the news at the given id*/
...
$sql  = "SELECT * FROM posts WHERE id='$SarchQueryParameter'";/*!< a variable that defines a sql statement to get all the news from posts table in the database with id='$SarchQueryParameter'*/
$stmt = $ConnectingDB ->query($sql);/*!< a variable that performs the sql statement by the database */
while ($DataRows=$stmt->fetch()) {/*!looping over all the rows in the posts table in the database to delete the news we want */
